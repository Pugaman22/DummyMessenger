...
class PG:
    ...
    async def initialize(self):
        ...
        async with self._pool.acquire() as conn:
            ...
            """)
            await conn.execute("CREATE UNIQUE INDEX IF NOT EXISTS unique_message_index ON messages (message_index);")
            await conn.execute("CREATE UNIQUE INDEX IF NOT EXISTS "
                               "unique_user_message_count ON messages (user_name, user_message_count);")
